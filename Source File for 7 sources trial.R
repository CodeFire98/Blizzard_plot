...
inputter = function(choice, choice1, choice2, valtemprb, valrhb, valwsb, valapb, valtemprant, valwsant, valmslpant, valtempr, valrh, valws, valap, date1, date2, date3, date4, date5, date6, date7) {
  ...
  if(choice2!="ant_tb3m") {
    if(choice2 == "dozerm") {
      ...
      subs = subset(subs, as.character(as.Date(subs$time_only, "%Y-%m-%d"))>=date[1] & as.character(as.Date(subs$time_only, "%Y-%m-%d"))<=date[2] & ap>900 & ws<50 & tempr<8 & tempr>-30)  
    } else {
  ...
  if(choice2=="dozerm") {
    subs$grp=factor(c(0, cumsum(diff(subs$time_only)>3600)))
  } else {
